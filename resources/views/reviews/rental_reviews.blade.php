            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-blue-50 text-gray-700 uppercase text-xs tracking-wider">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left font-bold">Item Rental</th>
                ...
                <tbody class="divide-y divide-gray-200 bg-white">
                    @foreach ($itemRvs as $index => $itemRv)
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                            <td class="px-4 py-4 text-center font-medium text-gray-900">{{ $index + $itemRvs->firstItem() }}</td>
                            <td class="px-4 py-4 text-gray-900 font-medium">{{ $itemRv->rizal_rental_items->name }}</td>
                            <td class="px-4 py-4 text-gray-900">{{ $itemRv->user->name }}</td>
                            <td class="px-4 py-4 text-yellow-500">{{ $itemRv->rating }} ★</td>
                            <td class="px-6 py-4 max-w-xs">
                                <div class="line-clamp-3">{{ $itemRv->comment }}</div>
                            </td>
                            <td class="px-4 py-4 text-green-500 font-medium">{{ $itemRv->like }}</td>
                            <td class="px-4 py-4 text-red-500 font-medium">{{ $itemRv->dislike }}</td>
